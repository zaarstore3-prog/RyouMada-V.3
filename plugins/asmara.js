...
export default {
  ...
  handler: async (sock, msg, from, sender, cmd, args, u, db, config) => {
    ...
    if (cmd === 'character' || cmd === 'char') {
      ...
      await sock.sendMessage(from, { text: "🔍 Sedang mencari data karakter di MyAnimeList..." });

      try {
        let charData = await fetchMalCharacter(query, db);
        if (!charData) {
          await sock.sendMessage(from, { text: "❌ Karakter tidak ditemukan di MyAnimeList." });
          return true;
        }

        let cleanDesc = charData.about ? charData.about.slice(0, 300) : 'Tidak ada deskripsi.';
        let txt = `🌸 D A T A  K A R A K T E R 🌸\n\n🆔 ID MAL: ${charData.mal_id}\n👤 Nama: ${charData.name}\n🇯🇵 Nama Jepang: ${charData.name_kanji || '-'}\n\n📝 Deskripsi:\n${cleanDesc}\n\n_Gunakan .lamar ${charData.mal_id} untuk menjadikan dia pasanganmu!_`;

        if (charData.images?.jpg?.image_url) {
          await sock.sendMessage(from, { image: { url: charData.images.jpg.image_url }, caption: txt });
        } else {
    ...
    if (cmd === 'pasangan' || cmd === 'ps') {
      ...
      if (fs.existsSync(pfPath)) {
        ...
        let charData = u.pasangan.id ? await fetchMalCharacter(u.pasangan.id.toString(), db) : null;
