...
import { fetchMalCharacter } from '../mal_cache.js';
import fs from 'fs';
...
export default {
  ...
  handler: async (sock, msg, from, sender, cmd, args, u, db, config) => {
    ...
    if (cmd === 'whitelistchar' || cmd === 'wlchar' || cmd === 'wl') {
      ...
      let id = args[0];
      if (!id) {
        await sock.sendMessage(from, { text: `❌ Format: ${prefix}whitelistchar [ID MAL]` });
        return true;
      }
      ...
      saveDB(db);
      await sock.sendMessage(from, { text: `✅ Karakter ID MAL ${id} berhasil ditambahkan ke Whitelist Global.` });
      return true;
    }
    ...
    if (cmd === 'delwhitelistchar' || cmd === 'delwlchar') {
      ...
      let id = args[0];
      if (!id) {
        await sock.sendMessage(from, { text: `❌ Format: ${prefix}delwhitelistchar [ID MAL]` });
        return true;
      }
      if (!db.global.whitelist_karakter) db.global.whitelist_karakter = [];
      ...
      saveDB(db);
      await sock.sendMessage(from, { text: `✅ Karakter ID MAL ${id} berhasil dihapus dari Whitelist Global.` });
      return true;
    }
    ...
    if (cmd === 'cekwhitelistchar' || cmd === 'cekwlchar') {
      ...
      await sock.sendMessage(from, { text: "🔍 Sedang memuat daftar karakter dari MyAnimeList... (Mohon tunggu sebentar)" });
      let txt = `📜 *DAFTAR WHITELIST GLOBAL*\n\n`;
      for (let id of wList) {
        try {
          // Jeda 1 detik per entri yg BELUM ke-cache -- supaya tidak membombardir
          // server MAL saat whitelist panjang & masih "dingin" (belum pernah discrape).
          let cacheKey = id.toString().trim().toLowerCase();
          let alreadyCached = !!db.global.mal_cache?.[cacheKey];
          let charData = /^\d+$/.test(id) ? await fetchMalCharacter(id, db) : null;
          if (!alreadyCached) await new Promise(resolve => setTimeout(resolve, 1000));
          let name = charData?.name || "Nama Tidak Diketahui";
          txt += `▸ ID MAL: ${id} - ${name}\n`;
        } catch (e) {
    ...
    if (cmd === 'delblacklist' || cmd === 'delbl') {
      ...
      if (!target || !db.users[target] || !targetId || isNaN(targetId)) {
        await sock.sendMessage(from, { text: `❌ Format: ${prefix}delblacklist [@tag/me] [ID MAL]` });
        return true;
      }
      let bList = db.users[target].blacklist_karakter || [];
      ...
      saveDB(db);
      await sock.sendMessage(from, { text: `✅ ID MAL ${targetId} berhasil dihapus dari blacklist @${target.split('@')[0]}`, mentions: [target] });
      return true;
    }
