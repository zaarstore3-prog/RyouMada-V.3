...
const OWNER_CMDS = new Set(['jadibot','jadibotacc','jadibotreject','killjadibot','jadibotlist','listjadibot','claimowner','resetglobal','fixekonomi','setaccgroup','antispam','ev','>','=>','exec','syncid','buatredeem']);
const ADMIN_CMDS = new Set(['menuadmin','setdata','add','delrole','delbadge','adddonate','infostaff','mute','unmute','addprem','delprem','ban','unban','whitelistchar','delwhitelistchar','cekwhitelistchar','cekblacklist','delblacklist','acc','tolak']);
const CMD_REGEX = /^[°•π÷×¶∆£¢€¥®™+✓_=|~!?@#$%^&.\/©^]/;
const PRIVATE_ALLOWED_CMDS = new Set(['jadibot', 'jadibotacc', 'jadibotreject', 'killjadibot', 'jadibotlist', 'listjadibot', 'report', 'saran', 'claimowner', 'acc', 'tolak']);
...
function scheduleReconnect(nomor, attemptFn, onGiveUp) {
  ...
  if (reconn.cancelled || reconn.timer) return;
...
async function startJadibotSession(mainSock, params) {
  ...
  if (!requester || !nomor || !sessionDir) return;
  
  let hasRequestedPairing = false;

  // 🔥 PERBAIKAN: Auth State diinisiasi SATU KALI di luar fungsi attemptConnect 
  // agar kunci enkripsi tidak tertimpa setiap kali reconnect loop berjalan
  try {
  ...
  const attemptConnect = async () => {
    ...
    try {
      ...
      jadibotSock = makeWASocket({
        ...
        browser: Browsers.ubuntu('Chrome'), // 🔥 Browser diganti ke standar Chrome agar WA tidak mendeteksi sebagai spam
...
export default {
  ...
  handler: async (sock, msg, from, sender, cmd, args, u, db, config) => {
    ...
    if (cmd === 'jadibotacc') {
      ...
      if (!requester) {
        await sock.sendMessage(from, { text: `❌ Data request rusak. Minta player mengulang ${prefix}jadibot.` });
        return true;
      }
