  ...
  handler: async (sock, msg, from, sender, cmd, args, u, db, config) => {
    ...
    if (cmd === 'claimowner') {
      if (db.global.owner_utama) {
        await sock.sendMessage(from, { text: "❌ Owner Utama sudah pernah diklaim sebelumnya dan tidak bisa diklaim ulang — termasuk oleh Owner yang sama. Ini mencegah penyalahgunaan tahta di kemudian hari." });
        return true;
      }
      db.global.owner_utama = sender;
      u.role = 'owner';
      saveDB(db);
      await sock.sendMessage(from, { text: "👑 *KLAIM BERHASIL* 👑\n\nAnda sekarang menjabat sebagai *Owner Utama*. Anda memiliki kekebalan mutlak di dalam sistem.\n\n_Klaim ini hanya bisa terjadi sekali seumur hidup bot — jaga baik-baik akses ini._" });
      return true;
    }
